...
연구 목적은 작업 설정 맨 위에서 고른다. A/A 허용 입력은 해당 목적에서만 보이며 연구 질문 오류의 위치로 이동할 수 있다. 연구 묶음·세션 메모는 선택 입력이고 기록 검색과 새 세션 준비를 지원한다. 결과의 ‘시행 포함 범위·연구 기록’에서 계획 전체의 유효·실패·미수행을 구분한다. 합성 진단은 별도 펼침 안에 두며 화면과 TXT로 결과를 읽는다. [새 사용법](RESEARCH-EXTENSIONS.md) · [디자인 자료와 검토](RESEARCH-UX-20260921.md)
