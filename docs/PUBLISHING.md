...
현재 앱 버전은 **v0.5.0 정식**입니다. UI·UX·아라·아샤 후속 개선과 벤치마크 방법론을 함께 포함합니다. 비교 대상 UnityBridge의 버전과 구분합니다.

## Desk와 리틀 모델 엔진의 개별 저장소

두 앱을 각각 올릴 때 Desk는 `build/Export-Source.ps1 -DeskOnly`, 독립 앱은 `build/Export-LittleModelEngine.ps1 -OutputDirectory '새 폴더 전체 경로'`를 사용한다. Desk 묶음에는 Companion 프로젝트를 제외하고, 리틀 모델 엔진은 자체 README·솔루션·코드·자산·검사·배포 스크립트를 제공한다. 이 작업은 업로드용 복사본만 만들며 원래 작업 폴더나 기존 Git 저장소를 삭제하지 않는다.

기존 Desk 저장소에 Companion 소스가 이미 올라가 있다면, 소스 파일을 덮어쓰는 것만으로 옛 폴더는 없어지지 않는다. 새 Desk 소스와 대조해 `src/UnityBridgeDesk.Companion`, `tests/UnityBridgeDesk.Companion.Tests`, `build/Publish-Companion.ps1`, `build/Verify-Companion.ps1`의 삭제 변경도 GitHub Desktop에서 확인한다. `.git`과 사용자 작업 파일은 보존한다.

현재 버전은 Desk v0.5.0, 리틀 모델 엔진 v0.1.0이다. 이미 같은 태그의 릴리스가 있으면 새 버전 번호를 임의로 만들지 말고 기존 릴리스의 수정 여부를 판단한다. 제공한 소스 ZIP의 내용물은 Code에, 실행용 ZIP은 Releases에 올린다.
...
| Releases → Assets | `UnityBridgeDesk-v0.5.0-win-x64.zip`, `SHA256SUMS.txt` |
| 릴리스 태그 | `v0.5.0` |
| 릴리스 제목 | `UnityBridge Desk v0.5.0 · UI·UX 개편과 근거 있는 벤치 분석` |
| 릴리스 본문 | [v0.5.0 본문](releases/v0.5.0.md)의 첫 제목 아래 내용 |
| GitHub Desktop Summary / Description | [커밋 문구](releases/v0.5.0-github-desktop.txt) |
...
2. 태그 `v0.5.0`, 제목 `UnityBridge Desk v0.5.0 · UI·UX 개편과 근거 있는 벤치 분석`을 입력합니다.
3. [릴리스 본문](releases/v0.5.0.md)을 붙여넣습니다.
4. 실행용 ZIP과 `SHA256SUMS.txt`를 첨부합니다.
5. **Pre-release는 선택하지 않습니다.** 정식 v0.5.0으로 게시합니다.

앱은 정식 배포이며 기본 측정 설정은 빠른 확인용입니다. 신뢰구간·반복 설계에는 충분한 상위 시행과 가정 점검이 필요합니다. README·릴리스 본문에 확인한 범위와 한계를 유지하세요. 공개 서명·설치 프로그램·자동 업데이트는 현재 제공하지 않습니다.
...
pwsh -File ./build/Export-Source.ps1 -DeskOnly
```
...
```powershell
Get-FileHash -LiteralPath './UnityBridgeDesk-v0.5.0-win-x64.zip' -Algorithm SHA256
```
