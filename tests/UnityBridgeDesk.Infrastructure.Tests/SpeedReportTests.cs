...
public sealed class SpeedReportTests
{
    ...
    [TestMethod]
    public void LegacyDiscoveryFailureShowsActualCallProgressWithoutRewritingEvidenceOrRetrying()
    {
        var run = SpeedReportSample.Create();
        var original = run.Results[0];
        var trial = original.Trial with { Experiment = "F02", Variant = "100" };
        string raw = "CLI 종료 코드 1 / 응답 형식: The input does not contain any JSON tokens. / " +
            JsonSerializer.Serialize(new { ok = false, error = "no active instance on port 8090" });
        var samples = Enumerable.Range(0, 18).Select(i => new SpeedSample(i, 900, 295, "success")).Append(
            new(18, 650, 0, "failed", "cli-error", Error: raw)).ToArray();
        var result = original with { Trial = trial, Status = "failed", Error = raw, FailureKind = "cli-error", FailureStage = "validation",
            Guest = original.Guest! with { Status = "failed", WorkMs = null, Error = raw, Samples = samples } };
        run = run with { Plan = [trial], Results = [result] };
        string before = JsonSerializer.Serialize(run);
        var report = new SpeedReport(run);
        var row = report.Trials.Single();
        Assert.Contains("계획 100개 중 19개 호출", row.Details);
        Assert.Contains("응답 검증 통과 18개", row.Details);
        Assert.Contains("첫 실패 19번째", row.Details);
        Assert.Contains("Unity 연결 검색 실패", row.Details);
        Assert.DoesNotContain("JSON tokens", row.Details);
        Assert.AreEqual("instance-unavailable", SpeedStability.Kind(row));
        Assert.IsFalse(row.Included); Assert.IsNull(row.WorkMs); Assert.AreEqual(0, report.Valid);
        Assert.AreEqual(1, report.Failed); Assert.AreEqual(1, report.Stability.Single().OtherFailures);
        Assert.AreEqual(before, JsonSerializer.Serialize(run));
        Assert.AreEqual(raw, result.Error);
        Assert.AreEqual("cleanup-error", SpeedStability.Kind(row with { Result = result with { Status = "cleanup-failed" } }));
        Assert.AreEqual("cancelled", SpeedStability.Kind(row with { Result = result with { Status = "cancelled" } }));
        Assert.AreEqual("timeout", SpeedFailure.ReportKind("timeout", raw));
        string unrelated = "compiler mentioned no active instance on port 8090";
        Assert.AreEqual(unrelated, SpeedFailure.Describe(unrelated));
    }
    ...
    public async Task ExportPreservesEvidenceReusesSnapshotsAndProtectsUserEditedWorkbook()
    {
        ...
        Assert.Contains(SpeedStatistics.MethodologyId, await File.ReadAllTextAsync(Path.Combine(first, SpeedReportFiles.MethodologyName)));
    ...
    public async Task WorkbookKeepsNumbersBlanksFiltersAndLiteralErrorText()
    {
        ...
        Assert.HasCount(8, Read("xl/workbook.xml").Descendants(S + "sheet").ToArray());
        for (int i = 1; i <= 5; i++)
        {
