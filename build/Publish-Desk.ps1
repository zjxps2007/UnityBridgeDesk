...
$deskEnvironment = @{ DOTNET_CLI_HOME=(Join-Path $deskRoot '.cache/dotnet'); NUGET_PACKAGES=(Join-Path $deskRoot '.cache/nuget'); DOTNET_CLI_TELEMETRY_OPTOUT='1'; DOTNET_GENERATE_ASPNET_CERTIFICATE='false' }
...
try {
    ...
    foreach($deskNotice in @('LICENSE','THIRD_PARTY_NOTICES.md')) {
        Copy-Item -LiteralPath (Join-Path $deskRoot $deskNotice) -Destination (Join-Path $deskOutput $deskNotice)
    }
    $deskNotices=Join-Path $deskRoot ".cache/nuget/microsoft.netcore.app.runtime.win-x64/$RuntimeVersion"
    ...
    @{ formatVersion=1; platform='win-x64'; selfContained=$true; runtime=$RuntimeVersion; sdk=(& dotnet --version); files=@($deskManifest) } | ConvertTo-Json -Depth 5 | Set-Content -LiteralPath (Join-Path $deskOutput 'distribution.json') -Encoding utf8
