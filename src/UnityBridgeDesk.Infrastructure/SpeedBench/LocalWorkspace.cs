...
public sealed record LocalSpeedSettings(string EditorPath = "", SpeedOptions? Options = null, int Palette = 0, string[]? SelectedTags = null, string? BaselineTag = null,
    OfficialUnitySelection? OfficialUnity = null, GoUnitySelection? GoUnity = null, bool ShowCompanion = true, bool AnimateInterface = true,
    bool CompanionRoaming = true, int CompanionActivity = 1, bool PlainScreen = false, bool? SidebarExpanded = null,
    bool ShowFoxCompanion = false, bool FoxRoaming = true, int FoxActivity = 1);
...
public static class LocalWorkspace
{
    ...
    public static string Parent(string dataRoot, string kind = "local-work") => kind is "local-work" or "official-work" or "go-work"
        ? Path.Combine(Path.GetFullPath(dataRoot), "speed", kind) : throw new ArgumentException("알 수 없는 실험 폴더 종류입니다.");
    public static async Task<string> Create(string dataRoot, Guid trial, string token, CancellationToken ct, string kind = "local-work")
    {
        ct.ThrowIfCancellationRequested();
        string root = Path.Combine(Parent(dataRoot, kind), trial.ToString("N")); SpeedFiles.Regular(root);
    ...
    public static async Task Check(string root, Guid trial, string token, string kind = "local-work")
    {
        SpeedFiles.Regular(root);
        if (kind is not ("local-work" or "official-work" or "go-work") || Path.GetFileName(Path.GetDirectoryName(root)) != kind || Path.GetFileName(root) != trial.ToString("N"))
    ...
    public static async Task Delete(string dataRoot, string root, Guid trial, string token, string kind = "local-work")
    {
        if (!InstanceDiscovery.SamePath(root, Path.Combine(Parent(dataRoot, kind), trial.ToString("N")))) throw new IOException("정리 대상이 관리 폴더를 벗어났습니다.");
        if (!Directory.Exists(root)) return;
        await Check(root, trial, token, kind);
    ...
    public static async Task Recover(string dataRoot, IProgress<string>? progress, CancellationToken ct, string kind = "local-work")
    {
        string parent = Parent(dataRoot, kind); SpeedFiles.Regular(parent);
        ...
        foreach (string root in Directory.EnumerateDirectories(parent))
        {
            ...
            var owner = File.Exists(marker) ? await SpeedFiles.Read<LocalWorkspaceOwner>(marker, ct) :
                ...
                kind == "local-work" ? await RecoverLegacyOwner(dataRoot, root, trial, ct) :
                throw new IOException("공식 도구 임시 폴더 소유 정보를 찾지 못해 보존했습니다: " + root);
            if (owner.TrialId != trial || owner.Token?.Length != 32)
                throw new IOException("실험 폴더 소유권이 다릅니다.");
            if ((kind != "local-work" || owner.Creator.Pid != System.Environment.ProcessId) && Alive(owner.Creator)) throw new IOException("다른 Desk 작업이 실험 공간을 사용 중입니다.");
            if (!File.Exists(marker)) await SpeedFiles.Write(marker, owner);
            await Check(root, owner.TrialId, owner.Token, kind);
            await CleanInstance(root);
            await GoUnityCli.CleanInstance(root);
            await Delete(dataRoot, root, owner.TrialId, owner.Token, kind);
        ...
        foreach (string journal in Directory.EnumerateFiles(parent, "*" + CleanupSuffix))
        {
            ...
            if ((kind != "local-work" || owner.Creator.Pid != System.Environment.ProcessId) && Alive(owner.Creator))
