...
public static class SpeedAnalysis
{
    public static SpeedSummary[] Summaries(SpeedRun run) => run.Plan.GroupBy(t => (t.Experiment, t.Variant, t.Tag)).Select(g =>
    {
        ...
        var values = trials.Where(Valid).Select(r => r.Guest!.WorkMs!.Value).Order().ToArray();
        ...
        return new SpeedSummary(g.Key.Experiment, g.Key.Variant, g.Key.Tag, g.Count(), values.Length, trials.Count(r => !Valid(r)),
    ...
    private static bool Valid(SpeedTrialResult result) => result.Status == "success" && result.ResetVerified && result.Guest is { Status: "success", WorkMs: > 0 };
    public static SpeedPair[] Pairs(SpeedRun run)
    {
        ...
        foreach (string candidate in run.Releases.Skip(1).Select(r => r.Tag))
        {
            ...
            foreach (var block in c.GroupBy(t => t.Block))
            {
                ...
                if (a is not null && b is not null && Valid(a) && Valid(b)) pairs.Add((a.Guest!.WorkMs!.Value, b.Guest!.WorkMs!.Value));
            }
