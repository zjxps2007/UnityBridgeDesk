...
public sealed record SpeedOptions(int Repeats = 2, int Warmups = 1, int Calls = 3, int TimeoutSeconds = 180,
    ...
    int StressRequests = 32, int StressConcurrency = 4, StressCommand[]? StressCommands = null, ResearchSettings? Research = null)
{
...
public sealed record SpeedRelease(string Tag, string Version, string Commit, string CliPath, string ConnectorPath,
    ...
    string? CliDirectory = null, string? CliTreeSha256 = null, string? AssetSha256 = null, OfficialUnityToolchain? OfficialUnity = null,
    GoUnityToolchain? GoUnity = null, string? SourceTag = null);
public sealed record GoUnityToolchain(string CliVersion, string ConnectorVersion, string ReleaseTag,
    OfficialUnityPackage[]? Packages = null, string? EditorVersion = null);
public sealed record OfficialUnityPackage(string Name, string Version, string Folder, string TreeSha256, string ArchiveSha256, string RegistrySha1, string Source = "registry");
public sealed record OfficialUnityToolchain(string CliVersion, string PipelineVersion, OfficialUnityPackage[] Packages, string? EditorVersion = null);
...
public sealed record GuestRequest(Guid RunId, SpeedTrial Trial, SpeedOptions Options, string UnityVersion,
    ...
    OfficialUnityToolchain? OfficialUnity = null, GoUnityToolchain? GoUnity = null);
...
public sealed record SpeedSample(int Index, double Milliseconds, int Bytes,
    ...
    [property: System.Text.Json.Serialization.JsonIgnore(Condition = System.Text.Json.Serialization.JsonIgnoreCondition.WhenWritingNull)] double? InnerDelayMs = null);
public sealed record GuestResult(Guid RunId, Guid TrialId, string GuestNonce, string Schema, string Status,
    ...
    [property: System.Text.Json.Serialization.JsonIgnore(Condition = System.Text.Json.Serialization.JsonIgnoreCondition.WhenWritingNull)] string? SceneEvidenceSha256 = null);
...
public sealed record SpeedRun(Guid Id, DateTimeOffset StartedAt, string Schema, string Evidence,
    ...
    SpeedTrialResult[] Results, string Status, string HostDescription, LocalEnvironment? Local = null, ToolWorkspaceCleanup? OfficialCleanup = null,
    ToolWorkspaceCleanup? GoCleanup = null, ResearchPlan? ResearchPlan = null);
public sealed record ToolWorkspaceCleanup(string Status, string Path, string? Error = null);
...
public static class SpeedProtocol
{
    ...
    public static SpeedTrial[] Schedule(SpeedOptions options, string[] tags)
    {
        ...
        var conditions = Cases(options);
        // Complete a randomized condition round before repeating it. Within each condition,
        // randomized Latin cycles balance target position without promising carry-over balance.
        var orders = conditions.ToDictionary(c => c, c =>
        {
            var rows = new List<string[]>();
            while (rows.Count < options.Repeats)
            {
                var basis = tags.ToArray(); random.Shuffle(basis);
                var offsets = Enumerable.Range(0, tags.Length).ToArray(); random.Shuffle(offsets);
                rows.AddRange(offsets.Select(offset => Enumerable.Range(0, tags.Length).Select(p => basis[(p + offset) % tags.Length]).ToArray()));
            }
            return rows.Take(options.Repeats).ToArray();
        });
        for (int round = 0; round < options.Repeats; round++)
        {
            var shuffled = conditions.ToArray(); random.Shuffle(shuffled);
            foreach (var condition in shuffled)
            {
                block++;
                foreach (string tag in orders[condition][round]) result.Add(new(Guid.NewGuid(), result.Count + 1, block,
                    tag, condition.Experiment, condition.Variant));
            }
        }
