...
public sealed class SpeedCoordinator(string dataRoot, VirtualBoxHost host)
{
    ...
    public static void ValidateResult(GuestRequest request, GuestResult result, string schema = SpeedProtocol.Schema)
    {
        ...
        if (result.Samples.Any(s => s is null || !double.IsFinite(s.Milliseconds) || s.Milliseconds < 0 || s.Bytes < 0 || s.Index < 0 ||
            ...
            result.WarmupMilliseconds is { } warmups && (warmups.Length > request.Options.Warmups || warmups.Any(v => !double.IsFinite(v) || v < 0)) ||
        ...
        int expectedWarmups = request.Trial.Experiment == "S01" || request.Trial.Variant == "first" ? 0 : request.Options.Warmups;
        if (result.WarmupMilliseconds is { } recordedWarmups && recordedWarmups.Length != expectedWarmups)
            throw new IOException("사전 실행 기록 수가 계획과 다릅니다.");
