...
public static class SpeedGuest
{
    ...
    private static async Task<GuestResult> Run(GuestRequest request, string root, CancellationToken ct, LocalTrialRequest? local = null)
    {
        ...
        var warmupMilliseconds = new List<double>();
        ...
        try
        {
            ...
            if (official && go) throw new InvalidDataException("하나의 시행에 비교 도구가 중복 지정되었습니다.");
            string cliDirectory = Path.Combine(root, "release", "cli"), cli = Path.Combine(cliDirectory, go ? "unity-cli.exe" : official ? "unity.exe" : "unity-bridge.exe"),
            ...
            if (!official && !go) CliDistribution.ValidateLayout(cliDirectory, request.CliIsBundle);
            ...
            stage = "compatibility";
            CliCapabilities? capabilities = null;
            if (official)
            {
                OfficialUnityCli.ValidateScenario(request);
                await OfficialUnityCli.Inspect(cli, root, request.OfficialUnity!, ct);
            }
            else if (go)
            {
                var packages = request.GoUnity!.Packages ?? throw new InvalidDataException("Go 의존 패키지를 다시 준비하세요.");
                if (!await GoUnityDependencies.Valid(packages.Select(p => p with { Folder = Path.Combine(root, "release", "packages", p.Name) }).ToArray(), ct))
                    throw new InvalidDataException("Go 의존 패키지 내용·버전이 다릅니다.");
                GoUnityCli.ValidateScenario(request);
                await GoUnityCli.Inspect(cli, root, request.GoUnity!, usesExec || request.Trial.Experiment == "S01", ct);
            }
            else capabilities = await CliCapabilities.Inspect(cli, root, usesExec, ct);
            stage = "preparation";
            if (official)
            {
                foreach (var package in request.OfficialUnity!.Packages)
                    if (await CliDistribution.TreeHash(Path.Combine(root, "release", "packages", package.Name), ct) != package.TreeSha256)
                        throw new InvalidDataException("게스트 Pipeline 의존 패키지 해시가 다릅니다: " + package.Name);
            }
            else if (go)
            {
                using var package = JsonDocument.Parse(await File.ReadAllTextAsync(Path.Combine(connector, "package.json"), ct));
                if (await CliDistribution.TreeHash(connector, ct) != request.ConnectorSha256 ||
                    package.RootElement.GetProperty("name").GetString() != GoUnityRepository.PackageName ||
                    package.RootElement.GetProperty("version").GetString() != request.GoUnity!.ConnectorVersion)
                    throw new InvalidDataException("Go Connector 내용·버전이 다릅니다.");
            }
            else
            {
            ...
            var dependencies = official ? request.OfficialUnity!.Packages.ToDictionary(p => p.Name,
                p => "file:" + Path.Combine(root, "release", "packages", p.Name).Replace('\\', '/')) : new Dictionary<string, string> {
                [go ? GoUnityRepository.PackageName : LocalInspector.ConnectorPackageName] = "file:" + connector.Replace('\\', '/') };
            if (go) foreach (var package in request.GoUnity!.Packages!)
                dependencies.Add(package.Name, "file:" + Path.Combine(root, "release", "packages", package.Name).Replace('\\', '/'));
            await SpeedFiles.Write(Path.Combine(project, "Packages", "manifest.json"), new { dependencies }, ct);
            await UnityEnvironment.InstallFixtureAsync(project, ct, fixture);
            if (official || go)
            {
                string installedFixture = Path.Combine(project, "Assets", "DeskBenchmark", "Editor", "DeskProbe.cs");
                await File.WriteAllTextAsync(installedFixture, (go ? "#define DESK_GO_CLI\n" : "#define DESK_PIPELINE\n") + await File.ReadAllTextAsync(fixture, ct), ct);
            }
            if (usesExec) await SpeedExec.WriteSource(root, ct);
            var runner = new TimedProcessRunner(); IInstanceDiscovery discovery = official ? new PipelineDiscovery() : new InstanceDiscovery(go ? GoUnityCli.InstancesDirectory : InstanceDiscovery.DefaultDirectory);
            ...
            if (before.UnityVersion != request.UnityVersion) throw new IOException("준비된 Editor가 보고한 Unity 버전이 계획과 다릅니다.");
            string lockFile = Path.Combine(project, "Packages", "packages-lock.json");
            if (File.Exists(lockFile)) File.Copy(lockFile, Path.Combine(root, "packages-lock.json"));
            Dictionary<string, string> CommandEnvironment() => go ? GoUnityCli.EnvironmentFor(root) : official ? OfficialUnityCli.EnvironmentFor(root) : new() { ["PYTHONIOENCODING"] = "utf-8", ["PYTHONUTF8"] = "1" };
            JsonElement ReadData(ProcessResult response, bool evaluatedCode = false) => go ? GoUnityCli.ReadData(response) : official ? OfficialUnityCli.ReadData(response, evaluatedCode) : SpeedFailure.ReadData(response);
            ...
            if (request.Trial.Experiment == "S01")
            {
                ...
                if (go) await GoUnityCli.PrepareConnection(root, discovery.Find(target, before.Port), request.GoUnity!, ct);
                ...
                var batch = await SpeedStress.Run(request.Options.StressRequests, request.Options.StressConcurrency, async (index, offset, token) =>
                {
                    ...
                    string[] args = go ? GoUnityCli.Arguments(project, request.Options.TimeoutSeconds, scenario.Command, JsonSerializer.Serialize(parameters)) : official ? OfficialUnityCli.Arguments(project, request.Options.TimeoutSeconds, scenario.Command, JsonSerializer.Serialize(parameters)) :
                        [..capabilities!.GlobalArguments(project, before.Port, InstanceDiscovery.DefaultDirectory, request.Options.TimeoutSeconds),
                    ...
                    var reply = await new TimedProcessRunner().RunAsync(new(Guid.NewGuid(), cli, [..args], project,
                        Environment: CommandEnvironment(), OutputCodePage: official || go ? 65001 : checked((int)GetACP())),
                    ...
                    try
                    {
                        var data = ReadData(reply, scenario.Command == "exec");
                        if (scenario.Command == "desk_probe")
                        {
                            var value = SpeedFailure.ValidateIdentity(data, nonce, before.Pid, project);
            ...
            (ProcessCommand Command, string Nonce) Prepare(string action, Dictionary<string, object>? values = null)
            {
                ...
                return (new(Guid.NewGuid(), cli, [..args], project, Environment: CommandEnvironment(),
                    OutputCodePage: official || go ? 65001 : checked((int)GetACP()), StandardInput: go && usesExec ? SpeedExec.Source : null), nonce);
            }
            JsonElement Validate(ProcessResult result, string nonce)
            {
                ...
                try
                {
                    return SpeedFailure.ValidateIdentity(ReadData(result, usesExec), nonce, before.Pid, project);
                }
            ...
            async Task<JsonElement> Untimed(string action, Dictionary<string, object>? values = null, bool warmup = false)
            {
                var call = Prepare(action, values);
                if (usesExec) await SpeedExec.WriteNonce(root, call.Nonce, ct);
                if (go) await GoUnityCli.PrepareConnection(root, discovery.Find(target, before.Port), request.GoUnity!, ct);
                var reply = await clock.RunAsync(call.Command, timeout, cancellationToken: ct);
                if (warmup) warmupMilliseconds.Add(reply.ElapsedMilliseconds);
                return Validate(reply, call.Nonce);
            }
            ...
            if (!(experiment is "F01" or "F04" && variant == "first"))
                ...
                {
                    var value = await Untimed(experiment == "F03" ? "payload" : "echo", experiment == "F03" ? new() { ["size"] = int.Parse(variant) } : null, warmup: true);
            ...
            if (go) await GoUnityCli.PrepareConnection(root, before, request.GoUnity!, ct);
            ...
            measurementMs = Stopwatch.GetElapsedTime(batchStart, batchEnd).TotalMilliseconds;
            stage = "validation";
